...
async function sendWhatsAppOTP(phoneNumber, otp) {
  const client = getTwilioClient();
  if (!client) {
    console.warn(`[DEV] Twilio not configured — OTP for ${phoneNumber}: ${otp}`);
    return;
  }

  const whatsappTo = `whatsapp:+91${phoneNumber}`;
  ...
  try {
    ...
    console.error(`WhatsApp OTP failed for ${whatsappTo}:`, err.message);
    console.warn(`[DEV] Falling back — OTP for ${phoneNumber}: ${otp}`);
    // Don't rethrow: let login proceed; OTP is logged above for dev use
  }
}
